Pro PSlit, wid, lambda= lam, asize= asz, colen= csz, hsize= hsz, tet_lim= tlm, $
	qscale = qsc, show_initial = shi, hi_prec = hpr, fwhm = fwhm, swid = swid,$
	transmission = trn, distance = dst, _extra = _e
...
;-
	...
	zeta1 = whsz/wcsz*sqrt((wasz*wcsz)^2 - 1)
	zeta2 = whsz^2*wlam/dst
	zeta = zeta1 + zeta2
...
;	if keyword_set(qsc) then qfac = 2*!pi*wwid else qfac = 1.
	...
	ht = Make_grid([0,1],p+1)

	hslint = Slitint(ht,sigma=sigh,zeta=zeta)
	xlim = (wid + 4*lam*dst/wid)/2.
	x = Make_grid([-xlim,xlim],lam*dst/wid,/step)
...
stop
	...
	dist = FPU_fix(sqrt(2/!pi)*wwid/(dst*sigh*qfac)*Real_mm(dist))
	xtet = dst*psi/(2*!pi*wwid)
	...
	if How_many(fir=fwhm,sec=swid,thi=trn,whi=whi) gt 0 then begin
		...
		if whi and 1 then begin
			print, 'FWHM, transmitted 	= ', Peak_fwhm(xtet,dist)
			if shifl then $
			print, 'FWHM, incoming		= ', Peak_fwhm(xtet,odist)
			print
		endif
		if (whi and 2)/2 then begin
			print, 'SWID, transmitted 	= ', Peak_swid(xtet,dist)
			if shifl then $
			print, 'SWID, incoming		= ', Peak_swid(xtet,odist)
			print
		endif
		if (whi and 4)/4 then begin
			print, 'Transmitted fraction	= ',Integ(xtet,dist,/val)
			print
		endif
	endif

	if Isnum(tlm) then begin
		dum = where(abs(xtet) le tlm, ndum)
	...
	if shifl then begin
		if lplt then Plotot,xtet,[[dist],[odist]],yran=yrn,line=[0,2],_extra=_e $
		else Plotot,xtet,[[dist],[odist]],line=[0,2],_extra=_e
	endif else plot, xtet, dist, _extra = _e
