Function PQR_post, fir, sec, type = typ, rpsq = rpsq, rqsq = rqsq, rr = rr
...
;		Modified 5-DEC-2019 by Mati Meron.  Internal changes.
;-
	...
	case n_params() of
		...
		1	:	begin
					...
					if n_elements(fdim) le 2 and fdim[0] eq 3 then begin
						...
						if pfl then rpsq = reform(res[0,*]) else rpsq = !null
						if qfl then rqsq = reform(res[1,*]) else rqsq = !null
						if rfl then rr = reform(res[2,*]) else rr = !null
