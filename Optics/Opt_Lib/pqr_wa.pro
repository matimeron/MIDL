Function PQR_wa, pqr, a, lam = lam, scale = sca, type = typ, tsq = tsq
...
;		8.61
; PURPOSE:
;		Evaluates angular aperture transformation of PQR format optical data.
; CATEGORY:
;		Optics calculation.
; CALLING SEQUENCE:
;		Result = PQR_WA( PQR, A [, keywords])
; INPUTS:
;	PQR
;		1D PQR data, i.e. a 3 element vector or a [3,N] array.
;	A
;		Angular aperture size, may be a scalar or a vector of length N, where N
;		is the second dimension of the PQR data (when present).
...
; OUTPUTS:
;		Returns the input PQR data transformed by an angular aperture of size A,
...
;-
	...
	if dim[0] eq 3 and n_elements(dim) le 2 then begin
		...
		wa = reform(Cast(a,5))
		if n_elements(wa) eq 1 then wa = wa[0] $
		else if n_elements(wa) ne dlen then message,'Dimensional inconsistency!'
		...
		tsq = wa^2/(wa^2 + (wpqr[0,*] + wpqr[2,*]^2)/wpqr[1,*])
		res[0,*] = tsq*wpqr[0,*] + (1-tsq)*wlam^2
		res[1,*] = (tsq*(wpqr[0,*] + tsq*wpqr[2,*]^2) + (1 - tsq)*wlam^2)/$
			(tsq*(wpqr[0,*] + wpqr[2,*]^2))*wpqr[1,*]
		res[2,*] = tsq*wpqr[2,*]
