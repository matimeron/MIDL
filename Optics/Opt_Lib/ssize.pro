Function Ssize, bsec, dum, internal = int, relative = rdum
...
;		4.2
; PURPOSE:
;		Calculates transverse *source* size of a beam.
; CATEGORY:
;		Optics calculation.
; CALLING SEQUENCE:
;		Result = SSIZE( BSEC [, INTERNAL = INT])
; INPUTS:
;	BSEC
...
; OUTPUTS:
;		Returns the source size of the beam, in microns (or in meters, if
;		/INTERNAL  is set).  If BSEC is a 2D array (see above), the
...
;-
	...
	return, FPU_fix(mult*sqrt(bsec[1,*]))
end