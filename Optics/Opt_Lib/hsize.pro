Function Hsize, bsec, dum, internal = int, relative = rdum
...
;		4.2
; PURPOSE:
;		Calculates transverse size of a beam.
; CATEGORY:
;		Optics calculation.
; CALLING SEQUENCE:
;		Result = HSIZE( BSEC [, INTERNAL = INT])
; INPUTS:
;	BSEC
...
; OUTPUTS:
;		Returns the size of the beam, in microns (or in meters, if /INTERNAL
...
;		None.
; PROCEDURE:
;		Straightforward.  Follows Eq. 3 in SOR.  Calls FPU_FIX from MIDL.
...
;-
	...
	return, FPU_fix(mult*sqrt(bsec[1,*] + bsec[2,*]*bsec[3,*]^2))
end