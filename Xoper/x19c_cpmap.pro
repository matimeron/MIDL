Pro X19C_cpmap, chiran, phiran, enpoints, title = tit, degrees = edeg, $
    theta_mono = tetmo, x_crystal = xcrys, totet_offset = totoff, beam = beam
...
;	X-ray optics
; CALLING SEQUENCE:
;	X19C_CPMAP [, CHIRAN, PHIRAN, ENPOINTS ][, keywords]
; INPUTS:
;    CHIRAN
...
; SIDE EFFECTS:
;	Changes values in the common blocks maintained in X19C_COMM.
; RESTRICTIONS:
;	At least one of the keyword parameters THETA_MONO, X_CRYSTAL, 
;	TOTET_OFFSET or BEAM should be used.
; PROCEDURE:
...
;	From XOPER:  X19C_COMM, SAMP_INTER.
...
;-
	...
	for i = 0, npoints do begin
		...
		X19C_comm, chi= -cpgrid(0,i,j), phi= cpgrid(1,i,j), degr = deg
		stot(i,j) = Samp_inter(y_sam = dum, degrees = deg)
		syot(i,j) = dum
	    endfor
	endfor
    endif

    if One_of(tetmo,xcrys,totoff,beam) eq -1 then $
    message, 'Some offset should be provided!'

    X19C_comm, /ini, degrees = deg, theta_mono = tetmo, x_crystal = xcrys, $
    totet_offset = totoff, beam = beam
    tot = stot
    yot = syot
    for i = 0, npoints do begin
	for j = 0, npoints do begin
	    X19C_comm, chi = -cpgrid(0,i,j), phi = cpgrid(1,i,j), degr = deg
	    tot(i,j) = Samp_inter(y_sam = dum, degrees = deg)
	    yot(i,j) = dum
	endfor
    endfor
    tot = tot - stot
    ...
    if n_elements(tetmo) ne 0 then begin
	ro = ro - step
	xyouts, lef, ro, 'Theta     = ' + string(tetmo,format = tform), /normal
    endif
    if n_elements(xcrys) ne 0 then begin
	ro = ro - step
	xyouts, lef, ro, 'X-Crystal = ' + string(xcrys, format=yform), /normal
    endif

    ro = ropos
    if n_elements(totoff) ne 0 then begin
	ro = ro - step
	out = '2-theta axis offset = ['
	for i = 0, 2 do out = out + string(totoff(i),format = tform)
	out = out + ']'
	...
	ro = ro - step
	out = 'Beam loc. offset = ['
	for i = 0, 2 do out = out + string(beam.loc(i) ,format = yform)
	out = out + ']'
	xyouts, rit, ro, out, /normal
	ro = ro - step
	dumd = beam.dir - [0,0,1]
