Pro TCRL_eval, energy = ene, rsig = rsg, asig = asg, und_length = udl, $
	...
	down_shift = dws, up_shift = ups, range = ran, hsize = hsi, vsize = vsi, $
	_extra = _e
...
;-
	...
	if Isnum(ran) then begin
		...
		if tfl then begin
			...
			hsi = transpose([[rran],[reform(rrsg[0,*])]])
			!p.region = [0,0,1,phi/2]
			plot, rran, rrsg[1,*], /noerase, tit= 'Vertical size', $
			xtit= xtit, ytit= ytit, _extra= _e
			vsi = transpose([[rran],[reform(rrsg[1,*])]])
