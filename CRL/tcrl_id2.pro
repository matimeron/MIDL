Pro TCRL_ID2, erange = ern, step = stp, rsig= rsg, asig= asg, und_length= udl,$
...
;-
	...
	if n_elements(ern) eq 2 then begin
		stp = Default(stp,0.01,/dtyp)
;		ene = Make_grid(ern,Default(stp,0.01,/dtyp),/step,dim=nen)
		ene = Make_grid(ern,stp,/step,dim=nen)
...
stop
	...
	Legend_mm, loc = 'ur', line = [0,0], text = ltext, col = [ccol,tcol],$
		thi = 2, /nowrap
