Pro ACRL_ID2, erange = ern, step = stp, rsig= rsg, asig= asg, und_length= udl,$
	elem = ele, radius = rad, thick = thi, aperture = apr, roughness = rof, $
	sorloc = sol, optloc = opl, samloc = sal,  mirloc = mil, slope_err = sle, $
	collimate = clm, hor_opt = hop, ver_only = vrt, limited = lim, fwhm = fwhm,$
	focloc = fcl, tranvals = etr, _extra = _e
...
;-
	...
	if n_elements(ern) eq 2 then begin
		...
		nen = nen[0]
		big = replicate(1/(machar()).epsneg^2,nen)
	...
	orsg = mult*sqrt(rsgsq + (l0+l1)^2*asgsq)
	oasg = mult*sqrt(asgsq)
	...
	if tfl then begin
		wfoc = transpose([[(1-vfl)*wfoc + vfl*big],[wfoc]])
		wsq = transpose([[(1-vfl)*wsq + vfl*big^2],[wsq]])
	endif
	sfac = sqrt(1 + qsq/wsq)
	...
	if Isnum(lim) then begin
		...
		thr = where(tlen gt lenmax, thnum,comp=cthr,ncomp=cthnum)
		thfl = thnum gt 0
		tlenf = tlen < lenmax
		tlens = tlen - tlenf
	endif else begin
	...
	if tfl then begin
		...
		if thfl then begin
			if vfl then begin
				wfoc[0,thr] = sfoc[thr]/tlens[thr]
				wsq[0,thr] = rad^2/(2*tlens[thr]*(mu[thr]*rad + dval[thr]))				
			endif else begin
				wfoc[0,thr] = sfoc[thr]/tlenf[thr]
				wsq[0,thr] = rad^2/(2*tlenf[thr]*(mu[thr]*rad + dval[thr]))		
			endelse
		endif
	endif
	sfac = sqrt(1 + qsq/wsq)
	ptran = Errorf_mm(apr*sfac/sqrt(8*qsq))/sfac
	if tfl then begin
		if vfl then ptran[0,cthr] = 1
		ttran = exp(-mug)*product(ptran,1,/pre)
	endif else ttran = exp(-mug)*ptran
