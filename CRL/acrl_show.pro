Pro ACRL_show, erange = ern, step = stp, rsig= rsg, asig= asg, und_length= udl,$
...
;-
	...
	else sal = Default(sal,56.0,/dtyp) > opl
	l0 = opl - sol
	l1 = sal - opl
...
;	endif
	...
	if Isnum(lim) then begin
		...
		thr = where(tlen gt lenmax, thnum)
		thfl = thnum gt 0
		tlenf = tlen < lenmax
		tlens = tlen - tlenf
	endif else thfl = 0
