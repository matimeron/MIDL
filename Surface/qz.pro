Function QZ, alp, bet, dth = dth, energy = ene, lam = lam, radians = rad
...
;		7.0
; PURPOSE:
;		Calculating the value(s) of Q_z.
; CATEGORY:
;		Surface specific.
; CALLING SEQUENCE:
;		Result = QZ ( ALP [, BET] [, DTH = DTH] {, ENERGY = ENE, LAM = LAM} $
;		[, RADIANS = RAD])
; INPUTS:
;	ALP
...
;		is zero.  DTH is a dummy variable here as the results don't depend on
;		it.  It is present for consistency with the QVEC and QXY routines.
;
...
;		Default is degrees.
; OUTPUTS:
;		Returns the Q_z value(s) corresponding to the input value(s).  The
...
;		Created 15-FEB-2007 by Mati Meron, as a front end to QVEC.
...
;-
	...
	return, Qvec(alp, bet,dth=dth,energy=ene,lam=lam,radians=rad,/qz)
end