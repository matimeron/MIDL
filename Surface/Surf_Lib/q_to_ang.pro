Function Q_to_ang, qx, qy, qz, lam = lam, energy = ene, k = kvl, qeps = eps, $
...
;		Modified 15-AUG-2013 by Mati Meron.  Internal changes.
;-
	...
	if Codims(wqx,wqy,wqz,dim=dim) then begin
		if n_elements(dim) eq 1 then begin
			...
			res = dblarr(3,dim>1)
			sta = intarr(dim>1)
			dum = where((abs(wqx) > abs(wqy) > abs(wqz)) gt weps*k, ndum)
