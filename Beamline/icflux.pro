Function ICflux, inp, ene, sensitivity = sen, inverse = inv
...
; INPUTS:
;	INP															| Either both
;		Either the ion chamber reading (as processed by the		| scalars, one
;		scaler) or, when /REVERSE is set, photon flux in ph/s.	| scalar and one
;	ENE															| vector or both
;		Photon energy, in keV.									| vectors of
;																| same length.
...
;		None.
; PROCEDURE:
;		Follows the derivation by Tim Graber.  Calls CODIMS, DEFAULT, FPU_FIX,
...
;		Modified 4-APR-2020 by Mati Meron.  Internal change, enabling two vector
;		inputs if they're of same length.
;-
