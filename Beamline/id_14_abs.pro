Function ID_14_abs, loc, focloc = foc, hang = han, vang = van, $
	abso= abs, abth= abt, show= sho, win= wwin, spot= spot, stat= stat, $
	out= pow, _extra= _e
	...
	if Streq(loc,'win',3) then begin
		abs = Default(abs,wind,/dtyp)
		abt = Default(abt,wnth)
	endif else if n_elements(abs) eq 0 or n_elements(abt) eq 0 then $
	message, 'Missing absorber and/or thickness!'
	...
	case (hfl + 2*vfl) of
		0	:	begin
					un_absorb, file = filf,.1, aloc, win = wwin, /sme, $
					filte = wfilt, filth = wflth, /no_sh, out = farf, $
					abso = abs, dept = abt
					un_absorb, file = fils,.1, aloc, win = wwin, /sme, $
