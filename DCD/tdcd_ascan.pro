Pro TDCD_ascan, ene, chiran = crn, qzran = qrn, step = stp, $
	...
	if whi eq 1 then begin
		if n_elements(dc1) eq 1 then begin
			wdc1 = amult*dc1[0]
			if bafl then wdc2 = - sin(phi1)/sin(phi2)*wdc1
	...
	if whi eq 2 then begin
		if n_elements(dc2) eq 1 then begin
			wdc2 = amult*dc2[0]
			if bafl then wdc1 = -sin(phi2)/sin(phi1)*wdc2
	...
	if Isnum(dq3) or bafl then begin
		if bafl then dq3 = -2*wdc1*sin(phi1)*flen
	...
	for i = 0, nchi-1 do begin
		...
		q1 = [0d,0,0]
		q2 = flen*[sin(2*phi1),cos(2*phi1),0]
