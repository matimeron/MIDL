Pro DCD_chi_pscan, ene, chiran = crn, qzran = qrn, step = stp, $
	length = len, degrees = deg, radians = rad, $
	p0_del = dp0, q1_del = dq1, q2_del = dq2, q3_del = dq3, $
	precise = pre, no_show = nos, layout = lay, compare = cmp, next = nxt, $
	...
	if not keyword_set(nos) then begin
		if mfl then begin
			...
			!p.region = pp[*,0]
			plot, pvar, loff, /nodata, /ynoz, xstyle= 1, $
				tit= ftit, xtit= xtit, ytit=ytit, _extra=_e
			oplot, pvar, loff, thi=2, col=pcol
			!p.region = pp[*,1]
			plot, pvar, toff, /nodata, /ynoz, xstyle= 1, /noerase, $
				tit= stit, xtit= xtit, ytit=ytit, _extra=_e
			oplot, pvar, toff, thi=2, col=pcol
			...
			if keyword_set(cmp) then begin
				...
				case whi of
					0	:	begin
								one = [sin(wchi),0,-cos(wchi)]/sin(wchi)
								fac = 1 - cos(spsi)
								two = [sin(wchi)*cos(wchi)*fac,0,$
								1-cos(wchi)^2*fac]/(sin(wchi)*sin(spsi))
								aoff = [Vinp(one,wdp0),Vinp(two,wdp0),0]
								print
								print, transpose(off), aoff
							end								
					1	:	begin
								aoff = sin(2*phi1)/(sin(phi1)*sin(spsi))*$
								[0,Vinp(wdq1,[-cos(phi1),sin(phi1),0]),0]
								print
								print, transpose(off), aoff
								print
							end
					2	:	begin
								aoff = sin(2*phi2)/(sin(phi2)*sin(spsi))*$
								[0,Vinp(wdq2,[cos(fpsi),-sin(fpsi),0]),0]
								print
								print, transpose(off), aoff
								print
							end
					3	:	begin
								aoff = wdq3[2]/(sin(wchi)*sin(spsi))*$
								[cos(wchi)*sin(spsi),-cos(spsi),0]
								print
								print, transpose(off), aoff
							end
					else:
