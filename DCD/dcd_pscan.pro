Pro DCD_pscan, ene, chiran = crn, qzran = qrn, step = stp, $
	...
	for i = 0, nchi-1 do begin
		...
		q1 = [0d,0,0] + wdq1
		q2 = $
		wlen*sin(spsi)/sin(2*phi2)*[sin(2*phi1),cos(2*phi1),0] + wdq2
	...
	if not keyword_set(nos) then begin
		if mfl then begin
			...
			!p.region = pp[*,0]
			plot, pvar, toff, /nodata, /ynoz, xstyle= 1, $
				tit= ftit, xtit= xtit, ytit=ytit, _extra=_e
			oplot, pvar, toff, thi=2, col=pcol
			!p.region = pp[*,1]
			plot, pvar, loff, /nodata, /ynoz, xstyle= 1, /noerase, $
