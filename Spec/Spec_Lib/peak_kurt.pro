Function Peak_kurt, x, y, ser, error = err, ret_range = ret, _extra = _e
...
;		4.9
; PURPOSE:
;       Calculating the kurtosis of a peak.
; CATEGORY:
;       Data analysis.
; CALLING SEQUENCE:
;       Result = PEAK_KURT ( X [, Y] [, SER] [, keywords])
; INPUTS:
;	X
...
; OUTPUTS:
;		Returns the kurtosis-value of a peak.
; OPTIONAL OUTPUT PARAMETERS:
;	ERROR
;		Returns the calculated statistical error of the kurtosis.
;	RET_RANGE
...
; PROCEDURE:
;		Calls PEAK_STRIP to affect background subtraction and PEAK_SCAMOM to
...
;-
	...
	res = Peak_scamom(sxy,ord=4,error=err) - 3
