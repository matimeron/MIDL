Function Scan_PD_comb, snum, fnum, verify = ver, orientation = ori, $
...
;		each column by by the ratio of reference field of view (at 90 degrees)
;		and the field of view corresponding to this column.  This corrects for
;		the changing field of view in GID scans.
...
;		SCAN_PD_ACOR, SCAN_PD_CENTER, SCAN_PD_FPREAD, SCAN_PD_FTOD and
;		SCAN_PD_FRAMES.  Calls ERREST from SURF_LIB.  Also calls DISPLAY_MM,
;		ISNUM and TYPE, from MIDL.
...
;		Modified 5-AUG-2016 by Mati Meron.  Internal changes to improve area
;		correction.
;-
	...
	if keyword_set(snr) and ori then $
	nco = nco*Scan_PD_acor(snum,fnum,hroi=hroi,_extra=_e)
