Pro Spec_file_info, filename = fnam, new = new, old_pars = olp, details = dtl, $
			...
			if njl eq 1 and jl lt jend[i] then begin
				...
				if njb ge 1 and jb[(njb-1)>0] lt jl then begin
					...
					if not fildat.pdfl then begin
						stf = ['','pilatus','apex','camera','','pilatus1m']
						sts = ['',' Pilatus','n Apex',' Camera','',' Pilatus1M']
					...
					case dtt of
						...
						4:	message, 'Unknown detector type!'
						5:	begin
								if pdini then begin
									fildat.pddim = [981,1043]
									fildat.pdpix = 0.172
								endif
							end
					endcase
