Function Scan_inter, scan, ival, sxlog=xlg, sylog=ylg, spline=spl, positive=pos
...
;	/POSITIVE
;		Switch.  In some rare cases SPLINE may generate negative values even
;		for purely positive SCAN input.  If that's the case then /POSITIVE 
;		forces the interpolation into linear mode.  If SPLINE is not set, 
...
;		linear interpolation.  Uses SCAN_SORT and, in some cases, calls itself
;		recursively.  Also calls DEFAULT, FPU_FIX, JOIN_XY, SPLIN_COEFFS,
;		SPLIN_EVAL and SPLIT_XY from MIDL.
...
;		default option.
;		Modified 15-AUG-2016 by Mati Meron.  Added keyword POSITIVE.
;-
	...
	if Default(spl,1,/dtyp) ne 0 then begin
		...
		if keyword_set(pos) and min(res[1,*]) lt 0 then $
		res = Scan_inter(scan,ival,sxlog=xlg,sylog=ylg,spline=0)
	endif else begin
