Function Scan_PD_comb_join, cfir, csec, $
...
;		SCAN_JOIN.
;		Modified 15-AUG-2016 by Mati Meron.  Internal changes.
;-
	...
	if Arreq([(size(cfir))[0:1],(size(csec))[0:1]],[3,4,3,4]) then begin
		...
		if rfl then begin
			...
			md = Scan_inter(lo,xhi[ovr],/pos,_extra=_e)
			...
			md = Scan_inter(hi,xlo[ovr],/pos,_extra=_e)
			noz = where(md[1,*] ne 0,nnoz,comp=zer,ncomp=nzer)
			if nzer gt 0 then begin
				if nnoz gt 0 then begin
					ovr = ovr[noz]
