Function Scan_find, snum, type = typ, stype = stp, astype = astp, $
...
;			Bsize		:	Beam size scan, primarily for use with CRL.
;
;	STYPE
...
;		Modified 25-DEC-2019 by Mati Meron.  Added Bsize data type.
;		Modified 10-FEB-2020 by Mati Meron.  Internal changes.
;-
	...
	posib = ['Detector','Direct','Reflectivity','XRF','Kscan','GIXOS',$
			'Inplane','GID','Pinhole','PGID','IH','SH','OSH','Bsize']
	poscod = [0,1,2,2,3,4,5,5,6,7,8,9,10,11]
	dstp = ['ascan','ascan','hklscan','hklscan','ascan',$
			'ascan','ascan','ascan','ascan','ascan','a2scan','ascan'] + '*'
	...
	case tcod of
		...
		11	:	begin
					lis = Scan_where(wsnum,stype=wstp,motor=['sh'],$
					/val,strict=str,aran=zran,bran=zran,dran=20*zran, $
					pran=[20,200],count=rcnt,_extra=_e)
					if rcnt eq 0 then $
					lis = Scan_where(wsnum,stype=wstp,motor=['shb'],$
					/val,strict=str,aran=zran,bran=zran,dran=20*zran, $
					pran=[20,200],count=rcnt,_extra=_e)
					if rcnt eq 0 then $
					lis = Scan_where(wsnum,stype=wstp,motor=['ih'],$
