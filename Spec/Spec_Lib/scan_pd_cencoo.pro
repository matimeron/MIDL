Function Scan_PD_cencoo, snum, fnum, verify = ver, raw = raw, angles = ang, $
...
;		SCAN_PD_FRAMES, SCAN_Q_ANGS and SPEC_FILE_CHECK.  Also calls DEFAULT,
;		ISNUM and ONE_OF, from MIDL.
...
;-
	...
	if wha then begin
		...
		pnfl = Default(pnh,min(angs[2,*]) gt 1,/dtyp)
		if pnfl then pdst = cur.pdsdist else pdst = !null
