Function Peak_int, x, y, ser, tight = tgh, refine = ref, $
	error = err, ret_range = ret, _extra = _e
...
;-
	...
	dx = Dif(wx,/cen,/lin)
	if keyword_set(tgh) or keyword_set(ref) then begin
		dx[0] = 0.5*dx[0]
		dx[nxy-1] = 0.5*dx[nxy-1]
	endif
	tres = total(wy*dx)
	if erfl then err = sqrt(total((wser*dx)^2)) else err = 0.
