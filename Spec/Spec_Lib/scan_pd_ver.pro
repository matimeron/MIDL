Function Scan_PD_ver, snum, fnum, warn = wrn, stat = sta
...
;-
	...
	tsiz = [4l*195*487,4l*981*1043] + 4096l
	wfl = keyword_set(wrn)
	sta = 0
	res = -1l
	...
	for i = 0, nfr-1 do begin
		...
		sver = file_info(rnam)
		starr[i] = sver.exists and $
		((sver.size eq tsiz[0])or (sver.size eq tsiz[1])or ((apfl mod 4) ne 0))
