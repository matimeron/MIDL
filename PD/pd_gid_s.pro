Pro PD_GID_S, snum, frame = fnum, verify = ver, angles = ang, $
...
;		Modified 10-NOV-2014 by Mati Meron.  Internal changes.
;-
	...
	slis = lis[sord]
	ystmin = Fltround(Scan_PD_ftod(slis[0],0,xdet=sli),dec=1,/ceil)

	whi = One_of(yli,nsp,/nozer)
	if whi ge 0 then begin
		if Isnum(yst) then begin
			wyst = abs(float(yst))
			if wyst gt 0 and wyst lt ystmin then begin $
				message, $
