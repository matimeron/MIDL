Pro PD_gixos_evolve, snum, fnum, bnum = bnum, bfnum = bfnm, verify = ver, $
	hroi = hroi, vroi = vroi, raw = raw, angle = ang, frames = fra, $
	t_reg = trg, z_reg = zrg, bin = bin, zbin = zbn, smooth = smo, $
...
;	ZBIN
;		Positive integer, specifies the bin size to be applied to the scan data,
;		in the vertical direction.  The binned data will have vertical dimension
;		of FLOOR(original_vertical_dimension/zbin).  The horizontal dimension is
;		not affected.
;	SMOOTH
...
;
;		Note:	When both ZBIN and SMOOTH are specified, the vertical binning is
;				performed before smoothing takes place.
;	/LAST
...
;		Modified 1-SEP-2015 by Mati Meron.  Added keyword ZBIN.
;-
	...
	if wbin gt 1 then begin
		...
		res[2,*,*] = rebin(reform(ores[2,*,*]),blen,siz[3])
		res[3,*,*] = sqrt(rebin(reform(ores[3,*,*])^2,blen,siz[3])/wbin)
	endif

	wbin = Default(zbn,1,/dtyp) > 1
	if wbin gt 1 then begin
		siz = size(res)
		blen = siz[3]/wbin
		ores = res[*,*,0:wbin*blen-1]
		res = fltarr(4,siz[2],blen)
		zco = rebin(reform(ores[1,0,*]),blen)
		res[0,*,*] = reform(ores[0,*,0])#replicate(1,blen)
		res[1,*,*] = replicate(1,siz[2])#zco
		res[2,*,*] = rebin(reform(ores[2,*,*]),siz[2],blen)
		res[3,*,*] = sqrt(rebin(reform(ores[3,*,*])^2,siz[2],blen)/wbin)
	endif
	...
	if keyword_set(plt) then begin
		dum = (Wherinstruct('min',_e))[0]
		if dum ge 0 then _e.(dum) = 0
		dum = (Wherinstruct('max',_e))[0]
		if dum ge 0 then _e.(dum) = 1e12
		window, (wnum - bwnum + 2) mod 4
		nrain = n_elements(!rainbow)
		ylfl = ((Wherinstruct('ylo',_e))[0] ge 0) $
			or ((Wherinstruct('log',_e))[0] ge 0)
		...
		plot, hco, tem[ihi,*]+ylfl, /nodata, ymar=[4,4],tit=tit[0]+'!c'+tit[1],$
			xtit= vtit[dtyp[1]], ytit= 'Counts', ylog= ylfl, _extra = _e
