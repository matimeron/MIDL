Pro PD_GID_xyint, snum, xy_reg = xyr, z_reg = zr, title = tit, log = log, $
	raw = raw, angles = ang, bare = bar, flip = fli, result = res, _extra = _e
...
; KEYWORD PARAMETERS:
;	XY_REG
;		A two element vector specifying Q_xy region of interest in [min,max]
...
;	/LOG
;		Switch.  Specifies logarithmic display (note that an offset may be added
;		to the diplayed result in this case).
;	/RAW
...
;	/FLIP
;		Switch.  If set, X and Y axes are exchanged, so that the plot points
;		'sideways".  Useful for some applications.
;	RESULT
...
;		where the first column contains the Q_xy-values, the second - the
...
;		None.
; PROCEDURE:
;		Straightforward, integrates the data over Q_xy (within the range
;		specified by keywords) through a call to PD_GID_INT, and diplays the
;		result through a call to SCAN_SHOW.  Calls DEFAULT, ERRBARS, ONE_OF and
;		TOLER from MIDL.
; MODIFICATION HISTORY:
;		Created 1-NOV-2007 by Mati Meron as a minor modification (changes only
;		in the input part) of LD_XYINT.
...
;-
	...
	res = PD_GID_int(snum,xy_reg=xyr,z_reg=zr,/xy_int,xy_ran=wxy,z_ran=wz,$
	...
	if keyword_set(bar) then begin
		...
		subtit= strjoin([otit+' range = '+swxy, xtit+' range = '+swz],'  ;  ')
	endelse
	...
	if keyword_set(fli) then begin
		plot, pres[1,*], pres[0,*], xlog = log, ymar = ymar, ystyle = 1, $
		tit = Default(tit, gtit), subtit = subtit, ytit = xtit, _extra = _e
		Errbars, pres[1,*], pres[0,*], xerr = pres[2,*]
	endif else Scan_show, pres, ylog = log, ymar = ymar, xstyle = 1, $
