Function PD_pin_range, snum, fnum, raw = raw, angles = ang, radians = rad, $
	span = spn, ranges = rng, footprint = ftl, _extra = _e
...
;	FOOTPRINT
;		Returns the observable footprint limits, in mm, as a 2-element vector.
; COMMON BLOCKS:
...
;		Modified 10-NOV-2013 by Mati Meron.  Added keyword FOOTPRINT.
;-
	...
	pix = round(cur.pdsdist*tan(psi)/cur.pdpix)
	cent = Scan_PD_center(wsnum)
	xpix = cent[0] + pix
	ypix = cent[1] + 0*pix
	ftl = lmlp*sin(psi[*,2])/sin(angs[2]+psi[*,2])
