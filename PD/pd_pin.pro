Pro PD_pin, sp_0, sp_1, sp_2, sp_3, sp_4, sp_5, sp_6, sp_7, frame = fnum, $
...
;		Modified 10-OCT-2015 by Mati Meron.  Bug fix.
;-
	...
	if keyword_set(lst) then begin
		...
		if (size(sp_0))[0] ne 3 then begin
			...
			for i = 0l, ii-1 do begin
				jlis = where(rbet eq rbet[ss[i]],jj)
				for j = 0, jj-1 do begin
					if (Scan_field_read(snum[jlis[j]],'varan'))[2] then begin
						klis = Scan_PD_frames(snum[jlis[j]],fnum,verify=ver,$
							nframes=kk,_extra=_e)
						for k = 0, kk-1 do begin
							if prfl then $
							print, snum[jlis[j]], klis[k], form = '(i6,i4)'
							if waul ne 0 then begin
								pran = PD_pin_range(snum[jlis[j]],klis[k],/raw)
								del = (pran[1]-pran[0])*(1-waul)
								hroi = pran + round(del*swei)
							endif
							ttem = $
							Scan_PD_read(snum[jlis[j]],klis[k],ver=ver,/indiv,$
						...
						if prfl then print, snum[jlis[j]], form = '(i6,i4)'
						if waul ne 0 then begin
							pran = PD_pin_range(snum[jlis[j]],0,/raw)
							del = (pran[1]-pran[0])*(1-waul)
							hroi = pran + round(del*swei)
						endif
						tem = Scan_PD_read(snum[jlis[j]],fnum,ver=ver,$
						hroi=hroi,vroi=vroi,/norm,raw= raw,angles= ang,$
