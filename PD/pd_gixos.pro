Pro PD_gixos, snum, noback = nbc, slit = sli, ssize = ssz, $
	angles = ang, shift = shf, scale = scl, tiny = tin, wnew = wnw, $
	show_factors = sho, result = res, sresult = sig, bresult = bac, $
	sfactor = sfc, bfactor = bfc, _extra = _e
...
;		Column 2	:	errors.
;	SRESULT
;		Returns the patched signal (no background subtraction) in same format as
;		RESULT.
;	BRESULT
;		Returns the patched background in the same format as RESULT.
;	SFACTOR
...
;		Modified 30-JUN-2016 by Mati Meron.  Added keywords SRESULT and BRESULT.
;-
...
stop
	...
	if bfl then begin
		...
		for i = 0, ns - 1 do begin
			...
			dat = Scan_scale(dat,cnorm/(bnum[i+1] - bnum[i]))
