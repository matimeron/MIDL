Pro PD_GID, sp_0, sp_1, sp_2, sp_3, sp_4, sp_5, sp_6, sp_7, $
...
;		SPEC.  Also calls ARRPACK, CLEAN_NAME, DEFAULT, DIF, DISPLAY_MM, ISNUM, 
;		LABELS, ONE_OF, SMOOTH_MM, TOLER, WHERINSTRUCT and WIMG_MM, from MIDL.
...
;		Modified 1-NOV-2015 by Mati Meron.  Internal changes.
;-
	...
	xch = reform(res[0,*,0])
	xpo = ceil((max(xch,min=minx) - minx)/(min(Dif(xch,/lin)) > Toler())) + 1
	dres = Img_equalize(res,/xonly,/down,sca=floor(auz[0]/xpo))
