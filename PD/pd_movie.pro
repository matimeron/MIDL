Pro PD_movie, snum, fnum, verify = ver, amax = amx, cmark= cmr, relative= rel,$
	big= big, small= sml, wait= wai, wnew= new, save= sav, file=fnam, rate=rat,$
	_extra= _e
...
;	/BIG
;		Switch.  If set, the display is stretched by roughly a factor of 2.
;	/SMALL
;		Switch.  If set, the display is shrunk by roughly a factor of 2.
;
;		Note:	Cannot set both /BIG and /SMALL at same time.
;	/WAIT
...
;-
	...
	win = keyword_set(new) or (sfl and (keyword_set(sml) or keyword_set(big)))
	...
	for i = 0l, nscan-1 do begin
		...
		for j = 0l, nfnum-1 do begin
			...
			cmark=wcmr, rel=rel, big= big,small=sml, expad=rpd*[8,4],shave=sml,$
			_extra= _e
			rpd = 0
			if sfl then begin
