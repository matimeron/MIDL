Pro UND_absorb, cur, dist, rsig, asig, file = fname, gscale = gsca, $
	...
	levels = slev, contour= con, surface= sur, show = sho, no_show = nosh, $
	wait = wai, save = sav, savfile = svname, outdata = odat
	...
	shabs = string(byte(absor)-byte([32,0,0,0]))
	for j = 0l, nz - 2 do begin
		...
		if keyword_set(sho) and shofl then begin
			...
			tit = tit[0] + tit[1] + shabs + ' abs. (power absorbed within ' + $
