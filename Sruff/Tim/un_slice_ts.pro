Pro UN_slice_ts, cur, dist, rsig, asig, file = fname, gscale = gsca, $
	...
	extra_process = ext, params = parm, levels = slev, dense = den, $
	surface = surf, shade_surf = shade,  wait = wai, outflux = oflux
	...
	for j = 0l, nsl - 1 do begin
		...
		tit(1) = tit(1) + $
			...
			'  (E/E!D1!N ='+strcompress(string(wsle(j)/ehar,form='(g9.4)'))+').'
		subtit(1) = subtit(1) + '.'

    ; *********************************************************************
    ; Define variables for common block to share with Sruff Pro Show_event
    ; added by Tim Stern 7/21/2003
    ...
    ; *********************************************************************
		...
		if keyword_set(shade) then begin
			shade_surf, farr, x, y, xtit = xtit, ytit = ytit, ztit = ztit, $
			title = tit(0) + tit(1), subtitle = subtit(0) + subtit(1)
		endif
