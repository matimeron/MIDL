Pro UN_absorb_ts, cur, dist, rsig, asig, file = fname, gscale = gsca, $
	smear = sme, window = win, tilt_ang = tang, xtilt = xti, degrees = deg, $
	filters = filts, filthicks = filths, mirrors = mirrs, mirangles = mirans, $
	extra_process = ext, params = parm, absorber = absor, depths = dep, $
	levels = slev, surface = surf, shade_surf = shade,  show = sho, wait = wai, $
	...
	for i = 1l, u.nh do begin
		...
		abco = 0.1*Abs_coeff(i*efac, elem = absor)
	...
	for j = 0l, nz - 2 do begin
		...
		if keyword_set(sho) then begin
			...
			if keyword_set(shade) then begin
				shade_surf, parr, x, y, xtit = xtit, ytit = ytit, ztit = ztit, $
				title = tit(0) + tit(1), subtitle = subtit(0) + subtit(1)
			endif
