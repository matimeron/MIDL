Pro UN_show_ts, cur, dist, rsig, asig, file = fname, gscale = gsca, $
	...
	wait = wai, no_show = nosh, outpower = opow, shade_surf = shade

	common un_consts, hcove, alpha, oovee, ecf
    common show_plot_info, glob, x, y, out_levs, lin_out, annot_out, xtit, $  ;T.S 7/15/03
                     ytit, ztit, title_out, subtitle_out   ;T.S 7/15/03
	...
	for i = 1l, u.nh do begin
		...
		if (where(shoset eq u.harms(i)) ge 0)(0) then begin
			if smefl then parr = Smear(parr,u,dist,/set_min,smearst=smest)
			parr = parr*frame
	...
	if smefl then glob = Smear(glob,u,dist,/set_min,smearst=smest)
	glob = glob*frame
    ...
    ; *********************************************************************
    ; Define variables for common block to share with Sruff Pro Show_event
    ; added by Tim Stern 7/15/2003
    ...
    ; *********************************************************************
		...
		if keyword_set(surf) eq 0 and keyword_set(shade) eq 0 then begin
