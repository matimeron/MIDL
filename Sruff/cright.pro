Pro CRight, radius = rad, _extra = _e
	...
	if Isnum(rad) then begin
		...
		dlp = len[3] - dl
		dlpp = dlp + len[4]
		dyp = dlp*cos(rang[3])/cos(rang[3]- rbasang)
		dypp = dlpp*cos(rang[3])/cos(rang[3]- rbasang)
		rx = (ry = fltarr(5))

		rx[0] = spox[4] - dl*sin(rang[2])/cos(rang[2]- rbasang)
		rx[1] = spox[4]
		rx[2] = spox[4] + dl*sin(rang[3])/cos(rang[3]- rbasang)

		ry[0] = ang[2]
		ry[1] = acos(cos(rang[2]) - (rx[1]-rx[0])/rad)*!radeg
		ry[2] = ang[3]

		ry[3] = asin(sin(rang[3]) + dyp/rad)*!radeg
		rx[3] = rx[2] + rad*(cos(rang[3]) - cos(!dtor*ry[3]))
		ry[4] = asin(sin(rang[3]) + dypp/rad)*!radeg
		rx[4] = rx[2] + rad*(cos(rang[3]) - cos(!dtor*ry[4]))
	...
	print, max(pox), max(rx), total(len) + 20
