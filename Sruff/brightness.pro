Function Brightness, ev, use = usnam, ering = elgev, current = cur, $
...
;       BRIGHTNESS
; PURPOSE:
;       Calculates on-axis brightness for a SXR source.
; CATEGORY:
;       X-ray calculations.
; CALLING SEQUENCE:
;       Result = BRIGHTNESS ( EV, keywords)
; INPUTS:
;    EV
...
; OUTPUTS:
;       Returns the calculated on axis brightness in units of
;       photons/sec/mrad^2[/per bandwidth or per eV].
...
; PROCEDURE:
;       Uses standard formulas to calculate brightness (see Jackson or Handbook
;	of Synchrotron radiation).  When the keyword USE is utilized, it is
...
;       BRIGHTNESS, the value of critical energy provided through ECRIT will be
...
;-
    ...
    ecrit = wecr, poles = npol
    wev = Evun(ev, wavelength = wav, kev_units = kev)
    relen = wev/wecr

    scale = mfac*Default(cur,1.)*Default(npol,1)*elgev^2*relen^2
    if n_elements(bw) eq 0 then scale = scale/ev else scale = scale*bw

    return, scale*Beselk_mm(0.5*relen,2./3.)^2
end
