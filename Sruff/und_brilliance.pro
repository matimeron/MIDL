Function UND_brilliance, ene, period = per, harm = har, bandwidth = ban, $
...
;		UND_BRIGHTNESS and UND_BEAMSIZE.  Calls CAST, HOW_MANY, NULLIFY and 
;		TYPE, from MIDL.
...
;-
	...
	scu=scu,ring_ene=ren,rgam=rgm,curr=cur,radial=rsg,angular=asg,$
	corr=crr,_extra=_e)
	wrsg = UND_beamsize(ene,rsize=rsg,asize=asg,und=udl,def=def,corr=crr, $
		har=har,per=per,scu=scu,rgam=rgm,_extra=_e)
