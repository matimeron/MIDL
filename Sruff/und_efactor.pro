Function UND_efactor, ene, ehr, emx, harmonic = har, nper = npr, period = per, $
	und_length = udl, scu = scu, def = def, scale = scl, _extra =_e
...
;-
	...
	case n_elements(per) of
		0	:	wper = floor(2*mult*udl/wnpr)/2d
		1	:	if per lt 1 then wper = mult*per else wper = 1d*per
		else:	message, 'Single or no value for period needed!'
	endcase
	if n_elements(emx) eq 0 then $
	ID_conv,per=wper,k=1d,emax=emx,scu=scu,_extra=_e else emx = Cast(emx,5)
	elim = whar*emx
