...
; PROCEDURE:
;		Follows the basic definitions OF SR quantities.  Uses BGP_CONV, 
;		BL_DEFAULTS and LOAD_FIELD_DATA from SRUFF_LIB, as well as ID_CONV_FUN
;		(from same location) for internal evaluations.  Calls CALCTYPE, CAST, 
;		HOW_MANY, IMAGINARY_MM, ISNUM, ONE_OF, ROOT, STRMATCH_MM, TABULATE, TYPE
...
;		Modified 15-JUL-2016 by Mati Meron, to make use of tabulated data
;		(courtesy of Roger Dejus, APS) instead of fits to data.
;-
	...
	if efl then ran = [0.5,1.5]*plim else ran = plim
	...
	case whi of
		3	:	begin
					bfl = Cast(bfl,typ)
					gap = Cast(gap,typ)
					per = BGP_conv(field=bfl,gap=gap,scu=sfl,_extra=_e)
					kvl = bfl*per/a
					emx = b/per
					ene = emx/(1 + kvl^2/2)
				end
		5	:	begin
					bfl = Cast(bfl,typ)
					per = Cast(per,typ)
					gap = BGP_conv(field=bfl,period=per,scu=sfl,_extra=_e)
					kvl = bfl*per/a
					emx = b/per
					ene = emx/(1 + kvl^2/2)
				end
		6	:	begin
					gap = Cast(gap,typ)
					per = Cast(per,typ)
					bfl = BGP_conv(gap=gap,period=per,scu=sfl,_extra=_e)
					kvl = bfl*per/a
					emx = b/per
					ene = emx/(1 + kvl^2/2)
				end
		...
		10	:	begin
					...
					for i = 0l, ndat - 1 do begin
						par[1] = par1[i]
						per[i]= Root('id_conv_fun',$
						ran,par=par,gap=gap[i],scu=sfl,stat=stat,_extra=_e)
		...
		34	:	begin
					...
					bfl = BGP_conv(per=per,gap=gap,scu=sfl,_extra=_e)
					kvl = bfl*per/a
					emx = b/per
				end
		36	:	begin
