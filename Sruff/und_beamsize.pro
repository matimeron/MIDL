Function UND_Beamsize, ene, rsize = rsg, asize = asg, $
	...
	ang_size = asz, ursig = ursg, uasig = uasg, _extra = _e
...
;		internal changes.
;		Modified 25-APR-2020 by Mati Meron.  Internal changes.
...
;-
	...
	awha = How_many(fir=rsg,sec=asg,whi=awhi)
	...
	if keyword_set(crr) then begin
		...
		ID_conv, gap=mgp, period=wper, emax=emx, rgam=rgm, scu=scu, _extra=_e
		emx = whar*emx
