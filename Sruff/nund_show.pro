Pro NUND_show, cur, dist, rsig, asig, file = fname, gscale = gsca, $
	...
	showset = shoset, pow_perh = poh, levels = slev, contour= con,surface= sur,$
	wait = wai, no_show = nosh, outpower = opow, enpow = epo, _extra=_e
	...
	for i = 1l, u.nh do begin
		...
		if (where(shoset eq u.harms(i)) ge 0)[0] then begin
			...
			peak = max(parr), smear = smest, tilt = tilst, surface = sur,$
			titl = tit, subtitl = subtit, xtit= xtit, ytit= ytit, ztit= ztit
			tit = tit[0] + tit[1] + 'Harmonic #' + $
			...
			Und_display,parr, x, y, con=keyword_set(con), sur=keyword_set(sur),$
