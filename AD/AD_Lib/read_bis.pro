Function Read_BIS, filename, rot, initialize = ini, nodata = nod, full = ful, $
...
;		Calls READ_BIS_HEADER.  Calls DEFAULT, DISPLAY_MM, FLTROUND, ISNUM, 
;		STREQ, STRMATCH_MM and STRPARSE_MM, from MIDL.
...
;		Modified 25-APR-2015 by Mati Meron.  Internal changes.
;-
	...
	if Isnum(filename) then begin
		rot = filename
		fnam = ''
	endif else fnam = Default(filename,'',/dtyp)

	hhead = Read_BIS_header(file=fnam,blknum=blknum,_extra=_e)
	...
	for i = 0l, n_elements(j) - 1 do begin
		...
		if loc ge 0 then begin
			lhead = vhead[loc]
			if fulfl then nexfl= ((where(exind eq j[i]))[0] eq -1) else nexfl= 1
	...
	if datfl then begin
		...
		tit = fnam, xtit = 'pix.', ytit = 'pix.', _extra = _e
