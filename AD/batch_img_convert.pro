Pro Batch_img_convert, file, inp_dir = idr, out_dir = odr, last = lst, $
	base = bas, reform = ref, image_type = imt, verbose = vrb, _extra =_e
...
; 	BASE
; 		Non-negative integer scalar.  If given, it'll be added to all the the 
; 		data files.
; 	/REFORM
...
;		STRMATCH_MM, STRPARSE_MM and TYPE, from MIDL.
; MODIFICATION HISTORY:
;		Created 5-MAY-2015 by Mati Meron.
;		Modified 5-SEP-2016 by Mati Meron.  Added keyword BASE.
;		Modifed 20-SEP-2016 by Mati Meron.  Added keyword REFORM.
;-
	...
	if nfil gt 0 then begin
		...
		for i = 0, nfil-1 do begin
			...
			form = '(a,"	",i0," of ",i0,a)'
			tem = Strparse_mm(fnam,'_',tlis)
			if tem ge 0 and strlen(tlis[tem]) lt 4 then begin
				tlis[tem] = strmid('000' + tlis[tem],3,/rev)
				fnam = strjoin(tlis,'_')
			endif
			out = odir + fnam + bext
			Img_to_sfrm, in= flist[i], out= out, base= bas, ref= ref, _extra= _e
		endfor
