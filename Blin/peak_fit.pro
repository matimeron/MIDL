Function Peak_fit, x, y, ser, fixed_err=fer, funct=fnc, edge=edg, comsig=csg,$
...
;		optional background.
;		Optionally fits "edges" to integrals of Gaussians or Lorentzians (i.e.
...
;	/COMSIG
;		Switch.  If set, the fitting is done with the constraint that all the
;		sigma values are the same.  For a single peak fitting COMSIG has no
;		effect (obviously).
;	REGION
...
;		Modified 10-APR-2016 by Mati Meron.  Added keyword COMSIG.
;-
	...
	if nc gt 1 then begin
		msk[ind+1] = 0
		val[ind+1] = ipar[ind+1]
		rpar = FGH_ext(fnam,feps,x_ini=ipar,par=wx,/min,mask=msk,val=val,$
		/sum,yvals=wy,wei=wei,stat=sta,_extra=_e)
		if keyword_set(csg) then begin
			boff = kb - btyp
			vec = lonarr(boff+nc-1,kb+kp*nc)
			val = dblarr(boff+nc-1)
			for i = 0l, boff-1 do begin
				vec[i,i+btyp] = 1
			endfor
			vec[boff:*,ind[0]+2] = 1
			if vfl then vec[boff:*,ind[0]+3] = 1
			for i = 0, nc-2 do begin
				vec[boff+i,ind[i+1]+2] = -1
				if vfl then vec[boff+i,ind[i+1]+3] = -1
			endfor
			msk=[]
		endif else msk[ind+1] = 1
		rpar=FGH_ext(fnam,feps,x_ini=rpar,par=wx,/min,mask=msk,vec=vec,val=val,$
		/sum,yvals=wy,wei=wei,error=err,stat=sta,chi=chi,covmat=cvm,_extra=_e)
	endif else 	rpar = FGH_ext(fnam,feps,x_ini=ipar,par=wx,mask=msk,val=val,$
