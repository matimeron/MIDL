Function Migauss_fun, p, q, deriv = der, grad = grd, hess = hes
...
;		8.07
; PURPOSE:
;		Evaluates a sum of Gaussian integrals with polynomial background.  
;		Primary purpose is to serve as evaluation function for fitting, in 
;		PEAK_FIT with EDGE option.
; CATEGORY:
;		Mathematical, general.
; CALLING SEQUENCE:
;		Result = MIGAUSS_FUN(P, Q [, keywords])
; INPUTS:
;	P
...
;		a Gaussian integral of P[3*N]*exp(-1/2*((Q - P[3*N+1])/P[3*N+2])^2).
;	Q
...
;		or more Gaussian integrals.  The dimension of the result is the 
;		dimension of Q.
...
;		Created 15-JUN-2011 by Mati Meron, based on MGAUSS_FUN.
;-
	...
	for i = 0l, ngau-1 do begin
		...
		if not Arreq(pp,[0,0,0]) then begin
			z = (wq - pp[1])/pp[2]
			f = FPU_fix(exp(-z^2/2))
			ff = Cast(sqrt(2*!dpi),typ,typ)*gaussint(z)		
			...
			if hfl then begin
				...
				hes[*,j+1,j+1] = -pp[0]/pp[2]*z*f
				hes[*,j+1,j+2] = (hes[*,j+2,j+1] = -pp[0]/pp[2]*z^2*f)
				hes[*,j+2,j+2] = -pp[0]/pp[2]*z^3*f
			endif
		endif
	endfor
