Function Img_trig_old, img, x, y, xsmall= xsm, ysmall= ysm, last=las, minimal=mnl,$
	xtr = rtx, ytr = rty
...
; KEYWORD PARAMETERS:
; 	/XSMALL
; 		Switch.  If set, the new X-coordinates are created with a spacing equal
; 		to the smallest spacing present in the original X-coordinates.  Note
; 		that this may change the number of points.
; 	/YSMALL
; 		Switch.  Same as XSMALL, for the Y-coordinates.
;	/LAST
...
;		ARREQ, CALCTYPE, CAST, FLTROUND, FPU_FIX, MAKE_GRID, SIGN and TYPE, 
;		from MIDL.
...
;-
	...
	if not (keyword_set(las) and Type(exs) gt 0) then begin
		...
		yran = ytem[ind]
		wsim = sim
		if keyword_set(xsm) or keyword_set(ysm) then begin
			if keyword_set(xsm) then begin
				dwx = Fltround(min(abs(Dif(wx[*,0],/lin))),dig=3)
				wsim[1] = round((max(xran,min=min) - min)/dwx) + 1
			endif
			if keyword_set(ysm) then begin
		...
		if chfl then begin
			...
			if checkx lt drx/2 or checky lt dry/2 then tring = 0 $
