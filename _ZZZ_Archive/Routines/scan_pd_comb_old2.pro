Function Scan_PD_comb_old2, snum, fnum, verify = ver, orientation = ori, $
	hroi = hroi, vroi = vroi, yoff= yof, norm= nrm, sin_norm= snr, other= oth, $
	regular= reg, show= sho, beta=bet, dth=dth, hrof=hrf, title=tit, _extra= _e
...
;		SCAN_PD_CENTER, SCAN_PD_FPREAD and SCAN_PD_FRAMES.  Calls ERREST from
;		SURF_LIB.  Also calls DISPLAY_MM, ISNUM and TYPE, from MIDL.
...
;-
	...
	apfl = (cur.pdstat - 3)/2
	wfnum = Scan_PD_frames(snum,fnum,ver=ver,nframes=nfnum,_extra=_e)
	ofl = keyword_set(oth)
	lo = 0l
					...
					if (cur.varan)[2] or ofl then begin
						...
						if Isnum(yof) then begin
							...
							if wof ne 0 then begin
								rdth = !dtor*dth
								tpsi = wof*sin(rdth)/$
								(cur.pdist - cur.pdsdist - wof*cos(rdth))
								dth = dth + !radeg*atan(tpsi)
								fof = round(cur.pdsdist*tpsi/cur.pdpix)
								fof = transpose([[fof],[lonarr(nfnum)]])
	...
	dat = Scan_PD_fpread($
	snum,wfnum,ori=ori,hroi=hroi,vroi=vroi,foff=fof,title=tit,_extra=_e)
	siz = size(dat)
