Pro Tune_curves_old, eran, period= per, harm_max= hrm, all_harm=all, bandwidth=ban,$
	def= def, scu= scu, min_gap= mgp, nper= npr, und_length= udl, current= cur,$
	ring_energy=ren, rgam=rgm, radial_size=rsg, angular_size=asg, correct=crr, $
	color= col, line= lin, thick= thi, alternate= alt, numbers=nmb, legend=leg,$
...
;-
				...
				if not afl then begin
					...
					if nfl then begin
						if ylfl then yoff = 0.02*bril[0] else yoff = 0.02*ymax
						 ...
						 string(har,form='(i0)'),col=!pcol.purple
					endif
				endif else begin
					tbril = gbril
	...
	if keyword_set(leg) then Legend_mm, text=string(wper,form='(f4.1,"mm")'),$
		line=wlin[0:numper-1],color=wcol[0:numper-1],ext=2
