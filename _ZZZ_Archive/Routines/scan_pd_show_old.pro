Pro Scan_PD_show_old, snum, fnum, filename= fnam, orientation= ori, display= dsp, $
	...
	small= sml,cmark= cmr, yoff= yof, result= res, ppos=pps, range=ran,_extra=_e
...
;	/SMALL
;		Switch.  If set, the display is shrunk by a factor of 2 (in each 
;		dimension).  Active only for image displays.
;	/CMARK
