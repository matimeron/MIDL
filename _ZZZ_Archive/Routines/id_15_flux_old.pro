Function ID_15_flux_old, ene, harmonic= har, station= stt, current= cur, slit= sli,$
	no_mirror = nom, display = dis, file = fil
		...
		for i = 0, nhar-1 do begin
			for j = 0, nlim-1 do begin
				...
				if ndum gt 0 then begin
					...
					res[i,dum] = Und_flux(ene[dum],per=33,har=whar[i],/quiet,$
					band=bw[j],/def,cur=wcur,/cor,/opt,ape=wape,dist=dist)
