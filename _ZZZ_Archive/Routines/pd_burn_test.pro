Pro PD_burn_test, snum, fnum, angles = ang, range = ran, brange = brn, $
...
;		Surface specific.
; CALLING SEQUENCE:
;		PD_BURN_TEST, SNUM [, FNUM] [, keywords])
; INPUTS:
;	SNUM
;		Single scan number.
...
; 		LINFIT_MM, ONE_OF and POLEVAL, from MIDL.
; MODIFICATION HISTORY:
;		CREATED 5-JUN-2014 by Mati Meron.
...
;-
	...
	if Arreq(reform(Scan_field_read(wsnum,'varan')),[0,0,0,0]) then begin
		...
		if keyword_set(ran) then begin
			pran = where(coo ge min(ran) and coo le max(ran),ndum)
