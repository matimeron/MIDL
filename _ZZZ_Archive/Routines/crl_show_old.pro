Pro CRL_show_old, erange = ern, step = stp, rsig= rsg, asig= asg, und_length= udl,$
...
;		ISNUM, LABELS, LEGEND_MM, MAKE_GRID, NULLIFY, PLVAR_KEEP, REAL_MM,
...
;-
	...
	sfoc = 1e-3*rad/alp
	if hfl then hvi = 0 else hvi = tfl
	ofoc = reform(1./(ncfl/(l1 > eps) + rvl[hvi,*]/qsq[hvi,*]))

	clen = sfoc/ofoc
	wfoc = ofoc
	if not cfl then $
	cfloc = reform(1./(1/wfoc - rvl[hvi,*]/qsq[hvi,*])) + opl
	...
	wfoc = sfoc/tlen
	if not cfl then $
	tfloc = reform(1./(1/wfoc - rvl[hvi,*]/qsq[hvi,*])) + opl
