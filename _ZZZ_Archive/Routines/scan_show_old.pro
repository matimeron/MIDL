Pro Scan_show_old, s_0, s_1, s_2, s_3, s_4, s_5, s_6, s_7, $
	s_8, s_9, s_10, s_11, s_12, s_13, s_14, s_15, $
	columns = cols, hoffset = hof, voffset = vof, scale = scl, extend = exn, $
	count = cnt, err_rat = ert, numbers = nmb, offnum = ofn, $
	line = line, lcolor = lcol, ynozero = ynoz, noerror = noe, wait = wai, $
...
;
;		Note:	If the number of line types or colors is smaller then the number
;				of scans, then the line types/colors are recycled.
;	/YNOZERO
...
;-
	...
	repeat begin
		...
		for i = 0, nsc - 1 do begin
			...
			oplot, x, y + lcfl, $
			line = lin[i mod nlin], col = lcl[i mod nlcl], _extra = _e
