Pro LS_process_old, file, inp_dir = idr, out_dir = odr, cut = cut, last = lst, $
...
;-
	...
	for i = 0, nfil - 1 do begin
		...
		form = '(a,"	",i0," of ",i0,a)'
		par = LS_fit(flist[i],cut=cut,cdata=cdat,error=err,check=chk,$
			show=plfl,_extra= _e)
		pars[[0,2,4],i] = par[3:*]
		pars[[1,3,5],i] = err[3:*]
		Wascii, cdat, odir + fnams[i] + '_bgs' + ext, /auto, call=2
		if simfl then $
		Wimg_mm, odir+ fnams[i]+ '_bgs' + pext, /jpg, qua=100, /auto, call=2
