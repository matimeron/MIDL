Pro Gentit_old, u, cur, dist, band, quantity = quan, total = tot, peak = pea, $
    ...
    if keyword_set(sur) then tish = string(tem) else tish = string(tem) + zti
    tit(0) = tish + ' ; K =' + strcompress(string(u.k, form = '(f7.3)')) + $
    ' ; I =' + strcompress(string(1000*cur, form="(i4,'mA')")) + $
    ' ; dist. =' + strcompress(string(dist, form = "(f7.3, 'm.')"))
    tit(1) = '!C'
