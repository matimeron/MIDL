Function UND_ecorr_old, ene, emx, harmonic= har, nper=npr, period=per,scu=scu, $
...
;-
	...
	if Isnum(emx) eq 0 then begin
		if not Isnum(per) then begin
			if wdef then begin
				...
				wper = floor(2d3*udl/npr)/2d3
			endif else message, 'Missing EMX, PER, DEF, cannot proceed!'
		endif else if per gt 1 then wper = 1d-3*per else wper = 1d*per
		ID_conv, per=1d3*wper, k=0.1d, emax=emx, scu=scu
	...
	if ranlim ge 5*edel/3 then begin
		...
		repeat begin
			...
			fac = $
			UND_efactor(ene,ehr,emx,harm=har,nper=npr,per=per,scu=scu,def=wdef)
		...
		fac= UND_efactor($
		ene,res,emx,harm=har,nper=npr,per=per,scu=scu,def=wdef,scale=0)
	endif else begin
		res = ene
		fac = 1
	endelse
