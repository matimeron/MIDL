Function RK4_int_old, dfun, range, eps, relative = rel, y_ini = yin, $
...
;	STEP													|
;		Initial integration step.							|	One and only one
;	NPOINTS													|	of these two
;		Number of evaluation points.  If given, the initial	|	must be given.
...
;-
	...
	wx = Cast(range[0],5)
	wy = Cast(yin,5)
	case One_of(stp,npt) of
		0	:	dx = sgn*abs(stp)
		1	:	dx = span/((npt>2) - 1)
	endcase
	atfl = keyword_set(atm)
