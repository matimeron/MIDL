Function ERoot, fun, range, eps, relative = rel, params = pars, multi = mult, $
...
;		6.4
; PURPOSE:
;		Finds roots of real functions.  Identical to the old routine ROOT except
;		for the fact that it allows for passing keywords to the function FUN
;		using _EXTRA.
; CATEGORY:
;		Mathematical function (general).
; CALLING SEQUENCE:
;		Result = EROOT( FUN, RANGE [, EPS [, keywords]])
; INPUTS:
;	FUN
...
;		None.
; RESTRICTIONS:
;		Since EROOT transfers keywords to the integrated function, it cannot
...
;		routine ROOT.  The modification consists of implementing keyword
;		transfer through _EXTRA.
;		Obsoleted 5-MAY-2014, since the new version of ROOT covers EROOT as
;		well.
;-
	...
	while ncur lt n_elements(rlis) and cmul ne 0 do begin
		...
		if (x[1] - x[0]) ge weps then begin
			f = x
			if pfl then begin
				f[0] = FPU_fix(call_function(fun,x[0],pars,_extra=_e))
				f[1] = FPU_fix(call_function(fun,x[1],pars,_extra=_e))
			endif else begin
				f[0] = FPU_fix(call_function(fun,x[0],_extra=_e))
				f[1] = FPU_fix(call_function(fun,x[1],_extra=_e))
			endelse
			x = x([0,1,1])
			f = f([0,1,1])
			if f[0]*f[1] gt 0 and mfl then begin
				...
				if pfl then $
				f[1] = FPU_fix(call_function(fun,x[1],pars,_extra=_e)) $
				else f[1] = FPU_fix(call_function(fun,x[1],_extra=_e))
				if f[0]*f[1] gt 0 then begin
					repeat begin
						...
						if pfl then $
						f[1]=FPU_fix(call_function(fun,x[1],pars,_extra=_e)) $
						else f[1] = call_function(fun,x[1],_extra=_e)
						if f[0]*f[1] gt 0 then begin
							comp = f[2]/f[1]
