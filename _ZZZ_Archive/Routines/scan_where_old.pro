Function Scan_where_old, snum, stype = stp, motors = mot, $
...
;		Modified 15-JUN-2015 by Mati Meron.  Adeed keyword NULL.
;-
	...
	if Type(stp) eq 7 then begin
		...
		if cnt gt 0 then begin
			if Streq(strmid(stp,0,/rev),'*') then begin
				...
				wstp = stp
			endelse
			ind = where(Streq(styps,wstp,slen),cnt)
