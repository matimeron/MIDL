Pro Top_flux_old, eran, estep, period = per, bandwidth = ban, $
	emin = emi, kmax = kma, nper = npr, und_length = udl, $
	current = cur, ring_energy = ren, rgam = rgm,  def = def, scu = scu, $
	radial_size = rsg, angular_size = asg, correct = crr, optimize = opt,  $
	aperture = ape, distance = dst, progress = prg, result = res, _extra = _e
...
;-
	...
	tit = 'Flux through a '+hape+'!9X!x'+vape+'mm aperture at '+sdst+'m'+'!c'
	...
	if weran[1] gt emn then begin
		...
		for i = 0l, nhar-1 do begin
			...
			if ndum gt 0 then begin
				tene = ene[dum]
				tres = Und_flux(tene,per=wper,har=ihar[i],ban=ban,def=wdef,$
				nper=npr,und_length= udl,scu= scu,ring_energy= ren,rgam= rgm,$
				current= cur,radial_size= rsg, angular_size= asg, correct= crr,$
				aperture = ape, distance = dst, optimize = opt)
				Nullify, whi, fir=ren, sec=rgm
