Function Img_join_old, id_0, id_1, id_2, id_3, id_4, id_5, id_6, id_7, $
	...
	if n gt 0 then begin
		...
		if vfl then begin
			...
			for i = 0, n-1 do begin
				...
				if erfl then ttem[3,*,*] = $
				Inter_2d(xgl,ygl[yloc],/vec,z_sor=tem,ind=3)
				otem = fltarr(3,yln)
				otem[0,*] = ygl[yloc]
			...
			for i = 1, n-1 do begin
				...
				if nov gt 0 then begin
					...
					if erfl then begin
						ulo = fir[2,*,p:p+nov-1]/(fir[3,*,p:p+nov-1]^2 > del^2)
						uhi = sec[2,*,0:nov-1]/(sec[3,*,0:nov-1]^2 > del^2)
			...
			for i = 0, n-1 do begin
				...
				if erfl then ttem[3,*,*]= $
				Inter_2d(xgl[xloc],ygl,/vec,z_sor=tem,ind=3)
				otem = fltarr(3,xln)
				otem[0,*] = xgl[xloc]
