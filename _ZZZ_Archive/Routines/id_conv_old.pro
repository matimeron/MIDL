Pro ID_conv_old, field= bfl, gap= gap, period= per, k= kvl, emax= emx, ene= ene, $
...
;		BL_DEFAULTS and ID_FIELD from SRUFF_LIB, as well as ID_CONV_FUN (from
;		same location) for internal evaluations.  Calls CALCTYPE, CAST, 
;		HOW_MANY, ISNUM, ONE_OF, ROOT, STRMATCH_MM, TABULATE and TYPE, from MIDL
...
;-
	...
	case whi of
		...
		10	:	begin
					...
					par1 = gap/(a*kvl)
					bfl = 0*par1
					dum = ID_field(1,/inv,scu=sfl,gap=gap,ran=ran,_extra=_e)
					par = [whi,0.,0.]
					for i = 0l, ndat - 1 do begin
						par[1] = par1[i]
						bfl[i]= Root('id_conv_fun',$
