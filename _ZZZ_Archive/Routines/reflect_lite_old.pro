Function Reflect_lite_old, theta = tet, qz = qzv, energy = ekev, wavelength = lamb,$
	dval = dvl, zval = zvl, thick = tarr, tanph = tph, roughness = roff, $
	s_dval = sdvl, degrees = deg, field = fld, rfnorm = rfn, ret_vals = rtv
...
;	TANPH
;		Numeric scalar, the ratio of the imaginary and real parts of the 
;		dielectric function.  Defaults to zero, i.e. no imaginary part 
;		(absorption).
;	ROUGHNESS
...
;-
	...
	case whi of
		...
		1	:	d = [0.,tarr,0.]
	endcase
	if n_elements(d) ne nlays + 1 then message, 'Wrong # of thicknesses!'
	d = d[0:nlays]
	...
	sangsq = sang^2 + complex(sfac*sdvl,-sfac*sdvl*wtph)
	for i = 1l, nlays do begin
		ni[*,i] = sqrt(sangsq - complex(sfac*dvl[i-1],-sfac*dvl[i-1]*wtph))
	...
	if not keyword_set(fld) then begin
		...
		dval=dvl[-1],tanph=tph,s_dval=sdvl,degrees=deg)
	endif
