Pro PD_Kscan_old, snum, slit = sli,back_ang = bag,left = lef,right = rig, $
...
;		SCAN_SCALE and SPEC_FILE_CHECK.  Also calls DEFAULT and RANGE_COMP, 
;		from MIDL.
...
;-
	...
	if max(Scan_field_read(lis,'PDfar')) eq 0 then begin
		if keyword_set(lct) then begin
			wtol = Default(tol,3*(fildat.scan[cnum].pdstat - 2))
			dct = Scan_PD_cenloc($
