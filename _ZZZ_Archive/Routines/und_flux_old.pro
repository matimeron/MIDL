Function UND_flux_old, ene, period = per, harm = har, quiet= qui, bandwidth= ban, $
...
;-
	...
	ID_conv, per = wper, gap = mgp, ene = emin, emax = emax, scu = scu
	if ene[0] lt whar*emin then ene[0] = ene[0]*(1+ 4*eps)
	if ene[-1] gt whar*emax then ene[-1] = ene[-1]*(1- 4*eps) 
	good= where(ene ge whar*emin and ene le whar*emax,ngood,comp=bad,ncomp=nbad)
	...
	if ngood gt 0 then begin
		...
		und=udl,scu=scu,ring_ene=ren,rgam=rgm,curr=cur,rad=rsg,ang=asg,corr=crr)
		wrsg = UND_beamsize(uene,rsize=rsg,asize=asg,und=udl,def=wdef,corr=crr,$
		har=whar,per=wper,scu=scu,rgam=rgm,ang=wasg,dist=dst)*sqrt(8d)
