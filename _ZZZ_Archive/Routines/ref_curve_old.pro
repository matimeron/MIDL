Function Ref_curve_old, rtet, energy = ekev, wavelength = lamb, $
	crystal = crs, index = ind, asym_angle = asym, $
	radians = rad, degrees = deg, corr_center = ccen, uncorr_center = ucen
...
;-
	...
	p = !pi/tan(tetb)*wtet
	q = Qval(cryst=crs,ind=ind,dref=dref,geo=gef)
