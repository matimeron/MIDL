Pro DCRL_show_old, erange = ern, step = stp, rsig= rsg, asig= asg, und_length= udl,$
...
;-
	...
	case wha of
		0	:	begin
					...
					if mifl eq -1 then begin
						...
						pqr2 = PQR_prop(pqrj,sle=[0,wsle],l=l[1]-lm)
					...
					if rfl then begin
						...
						if mifl eq -1 then begin
							...
							pqr2[*,fovr] = PQR_prop(pqrj,s=[0,wsle],l=l[1]-lm)
	...
	gtit = [!blpar.desc + mtit + ', undulator length ' + undlen,$
	...
	if mifl then begin
		mirloc = string(lm+ (1-mifl)/2*(l0+l[0]),form='(f5.2,"m")')
