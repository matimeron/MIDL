Pro PD_XR_old, snum, verify = ver, last = lst, abs_factor = abf, progress = prg, $
	...
	show_patches=shp, qcrit=qcr, rvrange=rvr, raw=raw, hrange=hrn, vrange=vrn, $
...
;		
;		Alternatively, QCRIT can be given as a 2-element vector.  In such case
;		PD_XR will attempt to find the QCRIT value best fitting the data and
;		this value will be used to evaluate the Fresnel-scaled reflectivity.
...
;		Modified 25-MAY-2015.  Added the option of fitting the data to find 
;		QCRIT.
;-
	...
	if Isnum(qcr) then begin
		if n_elements(qcr) eq 2 then begin
			wqcr = Qc_opt(res,qcr,sta=qsta)
			if qsta then com = '' else com = ' (Unreliable, no convergence)'
			print
			print, '	Fitted value of Qc = ', wqcr, com
			print
		endif else wqcr = qcr
	...
	if bagfl and keyword_set(raw) then begin
		...
		window, (cwin + 1 + Isnum(qcr)) mod 32
