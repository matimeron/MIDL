Function ABC_refc, abc, tet, chi, nvec = nvc, eta= eta, ref= ref, second= sec,$
...
; 	/SECOND
; 		Switch.  Signifies that the reflection is second of the same kind and
; 		prevents further reducing of the width of the distribution.
; 	/HOR_POL										|
...
;-
	...
	if Streq(tag_names(abc,/str),'abc') then begin
		...
		if keyword_set(sec) then res.con2 = res.con2 > 2*!dpi/weta^2 $
		else res.con2 = res.con2 + 2*!dpi/weta^2
