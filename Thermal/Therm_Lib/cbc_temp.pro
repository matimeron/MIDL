Function CBC_temp, d, t, blen, eps, nterms = ntr, precision= prc, net = net, $
	reset = rst
...
;		0.9
; PURPOSE:
;		Calculates the form-factor for circular, back cooled geometry.
; CATEGORY:
;		Thermal calculations.
; CALLING SEQUENCE:
;		Result = CBC_TEMP( D, T [, BLEN, EPS] [keywords])
; INPUTS:
;	D
;		Power load diameter.
;	T
...
;		purposes only.
; OUTPUTS:
;		Returns the value of the thermal form-factor for circular, back-cooled
;		geometry.
...
;		None.
; PROCEDURE:
;		Sums a series resulting from exact calculation.  Calls BC_ROOTS,
;		BESELK_MM, CAST, DEFAULT, SERIES_SUM, STRMATCH_MM, TOLER and TYPE,
;		from MIDL.
...
;-
	...
	if t gt 0 then begin
		asod = d*bc_rt/(2*t)
		ser =asod*Beselk_mm(asod,1,bc_eps)/(bc_rt^2*(1+sin(2*bc_rt)/(2*bc_rt)))
		...
		res = 8*t*res/d
	endif else if netfl then res = 0 else res = 4*wblen/d
