Pro Bounce_duu, angran, step= stp, crystal= crs, index= find, sindex= sind, $
	...
	if exs then begin
		...
		dang = Bragg_angle(ene=ene,crys=crs,ind=dind,dar=ddar)
		fang = Bragg_angle(ene=ene,crys=crs,ind=find,dar=fdar)
		sang = Bragg_angle(ene=ene,crys=crs,ind=sind,dar=sdar)
		bang = 2*(fang + sang - dang)
		val = where (bang ge angran[0] and bang le angran[1],nval)
		if nval gt 0 then begin
			ene = ene[val]
			bang = bang[val]
			wddar = 1e6*!dtor*ddar[val]
			wfdar = 1e6*!dtor*fdar[val]
			wsdar = 1e6*!dtor*sdar[val]
		endif else message, 'No angle/energy values in range!'
		dref = Ref_curve(0,ene=ene,crys=crs,ind=dind)
		fref = Ref_curve(0,ene=ene,crys=crs,ind=find)
		sref = Ref_curve(0,ene=ene,crys=crs,ind=sind)
		ref = dref*fref*sref
		...
		anfac = 1/(asiz*sqrt(1/asiz^2 + 1/wddar^2 + 1/wfdar^2 + 1/wsdar^2))
		...
		ftit = crnam + string(dind,form='("(",3i1,") down, ")') + $
		crnam + string(find,form='("(",3i1,")+")') + $
		crnam + string(sind,form='("(",3i1,") up")')
