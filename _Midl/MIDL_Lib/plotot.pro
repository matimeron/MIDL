...
;		Modified 20-MAY-2013 by Mati Meron.  Internal changes only.
;-
	...
	npsm = n_elements(psm)
	linst = Default(linst,0,/dtyp)
	nlin = n_elements(linst)
	col = Default(col,!p.color,/dtyp)
	ncol = n_elements(col)

	plvar_keep, act = 'sav'
	plot, px, py[*,0], xrange = [xmin,xmax], yrange = [ymin,ymax], $
		xtype = xtyp, ytype = ytyp, ynozero = ynz, /nodata, _extra = _e
	for i = 0, plnum - 1 do begin
		oplot, px, py[*,i], thick = thi, psym = psm[i mod npsm], $
		linestyle = linst[i mod nlin], color = col[i mod ncol]
	endfor
	plvar_keep, act = 'res'
