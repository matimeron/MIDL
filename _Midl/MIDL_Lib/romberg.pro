Function Romberg, fun, range, eps, infi = inf, relative = rel, params = par, $
...
;		Modified 20-SEP-2017 by Mati Meron.  Internal change.
;-
	...
	if ifl then begin
		if n_elements(range) le 1 then begin
			...
			if isdob then wrange = [wrange,!dpi/2] $
			else wrange = [wrange,!pi/2 - 2*Toler()]
