...
;		8.21
; PURPOSE:
;		Accepts data in form of a set (up to 32) one dimensional arrays and
;		prints it out as a table.
; CATEGORY:
;		Input/Output.
; CALLING SEQUENCE:
;		TABULATE, C_0 [,C_1 ....] [,keywords]
; INPUTS:
;	C_0, ....
;		One or more (up to 16) one dimensional arrays.  Type arbitrary
...
;		are C_0 through C_15.  The title of the index column, if it appears,
...
;	WIDTH
;		The page width (i.e. line length) used, in character #.  Default value
;		is the bigger of 80 and 10*number_of_columns.  Note that INDEX adds a
;		column when set.
;	/IDF_OFF
...
;		While in theory TABULATE can display up to 16 columns, the actual number
;		depends on the data types and magnitudes.  Approximate maximal numbers
;		of columns for page width of 80 are:
;			BYTE, INTEGER
...
;		Modified 30-APR-2011 by Mati Meron.  Added keyword WIDTH and some 
;		internal changes.
;		Modified 5-JUN-2011 by Mati Meron.  Internal changes.
;		Modified 25-AUG-2013 by Mati Meron.  Increased max # of columns to 32.
;-
	...
	linum = 22
	ulin = 95b
	cnams =  ['N',strcompress('C_' + sindgen(32),/remove)]
	...
	cinf = cinf[0:nco-1]
	linlen = Default(wid,nco*cowid,/dtyp) > dwid
