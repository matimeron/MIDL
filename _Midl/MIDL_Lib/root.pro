Function Root, fun, range, eps, relative = rel, params = par, multi = mult, $
...
;		Modified 25-OCT-2015 by Mati Meron.  Slight internal changes.
;-
	...
	weps = (feps = abs(Default(eps,deps,/dtype)))
		...
		if (x[1] - x[0]) ge weps then begin
			...
			if f[0]*f[1] gt 0 and mfl then begin
				...
				if f[0]*f[1] gt 0 then begin
					...
					endrep until csta or abs(x[2] - x[0]) lt 2*weps
				endif else csta = 1
		...
		if csta then begin
			if f[0]*f[1] eq 0 then begin
				...
				while cerv ge ceps do begin
					...
					if (x[1] eq x[0]) or (x[1] eq x[2]) then x[1]= (x[0]+x[2])/2
					f[1] = Rcall(fun,x[1],par,_extra=_e)
					if f[1] ne 0 then begin
						...
						if erat eq 1 then epco = 2*epco
						cerv = nerv
						ceps = 4*epco*feps*max(abs(x))
					endif else cerv = 0
				endwhile
