...
;		
;		Note:	String vectors are also acceptable.  If even one of the inputs
;				is of character type, all the inputs will be treated as such.
...
;		Modified 20-JAN-2014 by Mati Meron.  Internal changes, enabled sorting
;		of character arrays.
;-

	on_error, 1
	vnams =  strcompress('v_' + sindgen(8),/remove)
	typ = Calctype(v_0,v_1,v_2,v_3,v_4,v_5,v_6,v_7,def=3,/quiet)
	if typ eq 0 then begin
		for i = 0, 7 do dum = execute('typ = typ > Type(' + vnams[i] + ')')
		if typ ne 7 then message, 'Unacceptable input!'
	endif		
