...
;		Modified 15-NOV-2013 by Mati Meron.  Added keywords GRID, GSTYLE and 
;		EXPAD.
;		Modified 1-JAN-2014 by Mati Meron.  Internal changes.
;		Modified 1-DEC-2014 by Mati Meron.  Internal changes.
;-
	...
	case whi of
		0:	begin
				...
				mpof = [!d.x_size - xsiz + wepd[0], !d.y_size - ysiz + wepd[1]]
				...
				if fulfl then begin
					...
					xloi = (Wherinstruct('xlo',_e))[0]
					if xloi ge 0 then begin
						xlkeep = _e.(xloi)
						_e.(xloi) = 0
					endif
					yloi = (Wherinstruct('ylo',_e))[0]
					if yloi ge 0 then begin
						ylkeep = _e.(yloi)
						_e.(yloi) = 0
					endif
					...
					if xloi ge 0 then _e.(xloi) = xlkeep
					if yloi ge 0 then _e.(yloi) = ylkeep
				endif
				plvar_keep, act = 'res'
			end
		1:	begin
