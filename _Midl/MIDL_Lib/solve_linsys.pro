...
;		Modified 5-APR-2019 by Mati Meron.  Internal changes.
;-
	...
	if n_elements(rhs) ne siz[2-col] then message, 'Incompatible sizes!'
	...
	if keyword_set(svdfl) then begin
		tarr = arr
		trhs = rhs
		if col and (siz[1] lt siz[2]) then begin
			tarr = [tarr,fltarr(siz[2] - siz[1], siz[2])]
			trhs = [trhs,fltarr(siz[2] - siz[1])]
		endif
		if not col and (siz[1] gt siz[2]) then begin
			tarr = [[tarr],[fltarr(siz[1],siz[1]-siz[2])]]
			trhs = [trhs,fltarr(siz[1] - siz[2])]
		endif
