Function Polsolve, coef, alt_sort = alt
...
; KEYWORD PARAMETERS:
;	/ALT_SORT
;		Switch.  If set, changes the sorting order of the solutions.  By default
;		the solutions are sorted in (ascending) order of Real part, then 
;		Imaginary part.  If ALT_SORT is set, the sorting is on ABS(Imaginary),
;		then Real and then Imaginary.  This puts the real solutions (if they
;		exist) ahead of the complex ones.
; OUTPUTS:
...
;		Modified 20-OCT-2015 by Mati Meron.  Added keyword ALT_SORT.
;-
	...
	re = Real_mm(res)
	im = Imaginary_mm(res)
	if keyword_set(alt) then s= Lexisort(abs(im),re,im) else s= Lexisort(re,im)
