Function Centft, r, f, delta_r = dlr, zero_mean = zem, inverse = inv, $
	symm = sym, asymm = asym, nozero = noz, integral = int, positive_r = psr, $
	square_abs= sqa, join_xy= jxy, pack_result= pac, full_f= wf, coord= rok, $
	_extra = _e
...
;	/PACK
;		Switch.  Same as JOIN_XY, added for consistency with other routines.  
;	FULL_F
...
;		representation.
;		CENTFT calls ABS_MM, CAST, DEFAULT, ISNUM, ONE_OF and SPLIT_XY from MIDL
...
;		Modified 30-JUN-2014 by Mati Meron.  Added keywords SQUARE_ABS and
;		PACK_RESULT (a replacement for JOIN_XY).  Some minor internal changes,
;		copied over from CENTFT_2D.
;-
	...
	res = Cast(res,4,3 +(1 + dbtyp)*(3 - 2*sqfl),/fix)
	...
	if keyword_set(pac) or keyword_set(jxy) then res = transpose([[rok],[res]])
