Pro LD_patch_xy, s_0, s_1, s_2, s_3, s_4, s_5, s_6, s_7, $
...
;		7.15
; PURPOSE:
;		Patches together LD scans (horizontal orientation).
; CATEGORY:
;		Surface specific.
; CALLING SEQUENCE:
;		LD_PATCH_XY, S_0 [, S_1, ....] [,keywords]
; INPUTS:
;	S_0, S_1, ....
...
;				to WIMG_MM.
; OUTPUTS:
;		Standard output is graphics only, a plot of the patched data as a
;		function of Q_xy.  Additional outputs are provided through the output
;		parameters.
...
;-
	...
	res = LD_patch_gen(s_0, s_1, s_2, s_3, s_4, s_5, s_6, s_7, $
		orientation= 'hor', chan_range= chr, bin= bin, force= frc, $
