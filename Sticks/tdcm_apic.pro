Pro TDCM_apic, ang, voff= vof, loff= lof, aperture= ape, border= bor, _extra= _e

	on_error, 1

	Box, [-100,300], [-100,300], tru='ylo', xmar=[3,3], ymar=[2,2], border=bor
	...
	wof = vof/(2*cos(rang))
	hof = vof/(2*sin(rang))
	Rectan, xli = flen*[-0.75,0.25], yli=[-30,0], /no_sho, shape = fcr
	Rectan, xli = slen*[-0.5,0.5] + hof, yli = wof+[0,30], /no_sho, shape=scr
	rfcr = Shape_trans(fcr,rang)
	rscr = shape_trans(scr,rang)

	fsur = transpose([[flen*[-0.75,0.25]],[0,0]])
	ssur = transpose([[slen*[-0.5,0.5] + hof],[wof,wof]])
	afsur = transpose([[flen*[-0.75,-0.75]],[-30,0]])
	assur = transpose([[(-0.5*slen + hof)*[1,1]],[wof+[0,30]]])
	...
	Circle_mm, cent = [0,0], rad=100, thi=2, line=1
	...
	if rang gt 0 then begin
		...
		for i = 0, 2 do begin
			...
			if fcc then begin
				...
				if i eq 2 then plots, fcloc, psym=8, col=!pcol.cyan, $
					symsize=2, _extra = _e
