Pro Stick_xrf_glob, alp_range = alr, step = stp, $
	wait = wai, save = sav, file = fnam, rate = rat, _extra = _e

	on_error, 1
	brep = 30

	case n_elements(alr) of
		0	:	message, 'Missing ALPHA_RANGE input!'
		1	:	walr = [0,alr]
		else:	walr = alr[0:1]
	endcase
	if n_elements(stp) eq 0 then message, 'Missing step size!'
	alp = Make_grid(float(walr),stp,/step,dim=n)

	sfl = keyword_set(sav)
	if sfl then window, 5, xsi=640, ysi=512
	for i = 0l, n[0]-1 do begin
		Stick_xrf, alp = alp[i], _extra = _e
		if sfl then begin
