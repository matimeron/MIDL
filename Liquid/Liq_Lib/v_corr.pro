Function V_corr, dat, i, j, lag_range = lag, $
...
;		4.2
; PURPOSE:
;		Calculates velocity correlations for time-series data.
; CATEGORY:
;		Data Analysis.
; CALLING SEQUENCE:
;		Result = V_CORR( DAT [I, J [ keywords]])
; INPUTS:
;	DAT
...
; PROCEDURE:
;	 	V_CORR is simply a front-end for DAT_CORR (with /VELOCITY set).  See
;		details there.
...
;-
	...
	return, Dat_corr(dat,i,j,lag=lag,/vel,lval=lval,lcou=lcon,var=var,_extra=_e)
end